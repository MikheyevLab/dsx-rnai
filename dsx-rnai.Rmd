...
```{r go, warning = FALSE}
go <- read_tsv("data/go.tsv", col_types = c()) %>% mutate(evidence = "ISS") %>% dplyr::select(GO=go, evidence,gene=gene_id)
...
```{r goTable}
summary(DsxModuleGo) %>% kable("html") %>% kable_styling() %>%  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = T) %>% scroll_box(height = "400px") 
```
