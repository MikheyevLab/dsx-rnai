...
library(RRHO)     # correlation between ranked gene lists EXPERIMENTAL
...
bothModules <- left_join(deHe.tgw$table[heNet$colors == heDsxModule,],de.tgw$table[net$colors == DsxModule,] %>% dplyr::select(gene_id, logFC), by = "gene_id") %>% na.omit()
with(bothModules, cor.test(logFC.x, logFC.y, method="s"))
```
...
```{r RRHO }
rrho.test <- RRHO(bothModules[,c("gene_id", "logFC.x")], bothModules[,c("gene_id", "logFC.y")], BY=TRUE, alternative='two.sided', labels = c("developmenta", "experimental"))
set.seed(123)
rhho.test.perm <- pvalRRHO(rrho.test, 1000)
rhho.test.perm$pval
```
...
ggsave("figures/qp ovary.pdf", height=5, width=5)
