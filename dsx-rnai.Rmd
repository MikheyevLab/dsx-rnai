...
library(networkD3) # force network
...
```{r captioner, echo = FALSE, message = FALSE, warning = FALSE, cache=FALSE}
library(captioner)
...
figs(name = "pheroCorr", caption = "Correlation matrix between individual pheromone components")
...
de.tgw <- exactTest( cds , pair = c( "DSX" , "GFP" ) ) # positive logFC means higher levels in control
...
de.tgw$table %>% filter(gene_id  == "LOC413289") # Duncan et al. Inhibition of notch makes workers reproduce
```
#### `r figs("knockdwn")`
...
```{r qp}
qp <- read_tsv("data/Honey bee QMP compounds_wide.txt", col_types = cols())
...
colnames(M) <- rownames(M) <- c( "10-HDAA", "9-HDA", "HOB", "10-HDA", "HVA", "9-ODA")
...
p.mat <- cor.mtest(qp[, 1:6]) # p-value correlation matrix

col <- colorRampPalette(c("#4477AA", "#77AADD", "#FFFFFF", "#EE9988","#BB4444"))

corrplot::corrplot(M, method="color", col=col(200),  
         type="lower", order="hclust", outline=T, mar = c(1, 1, 1, 7),
         addCoef.col = "black", tl.col="black", tl.srt=80, p.mat = p.mat, sig.level = 0.01, insig = "n",diag=FALSE)
```
...
connectivityDsxModule[connectivityDsxModule$gene_id == "Dnmt3","interestingLabes"] <- "Dnmt3"
...
This network shows the 30 genes most highly connected to _Dsx_, with edge weights proportional to the strength of the connection, and node areas to log counts per million.

```{r networkPlotSetup, echo = FALSE}
...
deHeDay4.tgw <- exactTest( cdsHeDay4 , pair = c( "worker" , "queen" ) ) # queen has  positive logFC
deHeDay4.tgw$table$gene_id <- heCounts$gene_id
deHeDay4.tgw$table <- left_join(deHeDay4.tgw$table, descriptions, by = "gene_id") 


# module membership in He et al Dsx module in He et al data
cor.test(heAlldegrees$kWithin[heNet$colors == heDsxModule], deHeDay4.tgw$table$logFC[heNet$colors == heDsxModule], method = "s")

# module membership in He et al Dsx module in He et al data for shared module
cor.test(heAlldegrees$kWithin[net$colors == DsxModule & heNet$colors == heDsxModule], deHeDay4.tgw$table$logFC[net$colors == DsxModule & heNet$colors == heDsxModule], method = "s")
...
## Comparison with sex bias

A recent study by [Warner et al.](https://www.biorxiv.org/content/early/2018/11/20/454645) found that caste biased genes are associated with sex bias, specifically that queen-biased genes (vs workers) are also more female-biased (vs males). This is interesting, since _Dsx_ proximally controls sex bias, suggesting that the _Dsx_-responsive module may be involved in caste bias as well.

```{r cache=FALSE}
heFactors <- read_tsv("data/he_factors.tsv", col_types = cols()) %>% filter(caste != "worker")
design <- model.matrix(~caste, heFactors)
cdsHe <- DGEList( heCounts[,  heFactors$sample] , group = heFactors$sex)
cdsHe <- calcNormFactors( cdsHe) 
cdsHe <- estimateDisp( cdsHe, design)
deHeSex.tgw <- exactTest( cdsHe , pair = c( "male" , "female" ) ) # queens have positive logFC
deHeSex.tgw$table$gene_id <- heCounts$gene_id
deHeSex.tgw$table <- left_join(deHeSex.tgw$table, descriptions, by = "gene_id") 

cor.test(deHeSex.tgw$table[net$colors == DsxModule,"logFC"], de.tgw$table[net$colors == DsxModule,"logFC"])

cor.test(deHeSex.tgw$table$logFC, de.tgw$table$logFC, method="s")

dmel <- read_csv("data/dmel_sexGenes.csv", col_types = cols()) %>% mutate(logFC = -logFC) # data from Warner et al. logFC is female/male, so we swith them to match analysis above

with(left_join(deHeSex.tgw$table, dmel, by = c("gene_id" = "gene_Amel")), cor.test(logFC.x, logFC.y)) # overall the direction of sex bias is conserved, as expected

with(left_join(de.tgw$table[net$colors == DsxModule,], dmel, by = c("gene_id" = "gene_Amel")), cor.test(logFC.x, logFC.y, method = "s"))

with(left_join(de.tgw$table, dmel, by = c("gene_id" = "gene_Amel")), cor.test(logFC.x, logFC.y, method = "s"))

left_join(de.tgw$table[net$colors == DsxModule,], dmel, by = c("gene_id" = "gene_Amel")) %>% ggplot(aes(logFC.x, logFC.y)) + geom_point() + xlim(-1,2) + geom_smooth(se=F, method="rlm")+xlab("Log-fold count experimental treatment (control/knockdown)") + ylab("Log-fold count fruit fly sex bias (female/male)")+theme_bw()
```

Suprisingly, both the ant and fruit fly gene expression patterns are negatively correlated with those induced by _Dsx_-sensitive module expression. Control bees are more similar to males than they are to 
