...
```{r kallistoSleuthTable, cache.rebuild=!file.exists("data/sleuth_table.rds")}
...
```{r wgcna, cache.rebuild=!file.exists("data/net.rds")}
...
```{r dsxModuleGenes}
dsxModuleGenes <- descriptions %>% filter(gene_id %in% colnames(expDat)[net$colors == DsxModule] ) %>% left_join(de.tgw$table, by = "gene_id") %>% left_join(connectivityDsxModule %>% select(gene_id, kWithin), by="gene_id") %>% select(gene = gene_id, description = description.x, logFC, kWithin, logCPM) %>% arrange(desc(logFC))  
...
forceNetwork(Links = edges, Nodes = nodes,
            ...
            Value = "value", NodeID = "description",  fontSize = 18, colourScale = JS("d3.scaleOrdinal(d3.schemeCategory20);"), linkDistance = 100, linkWidth =  JS("function(d) { return d.value; }"), Nodesize = "logCPM", radiusCalculation = JS("d.nodesize+3"), fontFamily = "arial", Group = "color", opacity = 0.8, width = 500, height = 500)
```
