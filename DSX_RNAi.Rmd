...
```{r captioner, echo = FALSE, message=FALSE}
...
dsxPlot <- data.frame(tpm = t(subset(tpm, gene_id == "Dsx")[,as.character(treatments$sample)]) , trt = treatments$treatment) %>% ggplot(aes(trt,tpm))+geom_boxplot() + geom_signif(comparisons = list(c("DSX", "GFP")), test = "dsxTest") + xlab("dsRNA injected")+ylab("TPM")
...
bm<-read_tsv("https://github.com/marivelasque/dsx-rnai/raw/master/data/DSX_RNAi_Mortality.txt")
...
head(round(M,2))
colnames(M) <- c( "10-HDAA", "9-HDA", "HOB", "10-HDA", "HVA", "9-ODA")
rownames(M) <- c( "10-HDAA", "9-HDA", "HOB", "10-HDA", "HVA", "9-ODA")
...
corrplot(M, method="color", col=col(200),  
         ...
         tl.col="black", tl.srt=80, #Text label color and rotation
         # Combine with significance
         p.mat = p.mat, sig.level = 0.01, insig = "n", 
         # hide correlation coefficient on the principal diagonal
         diag=FALSE
)
```

# Ovary and pheromonal activation analysis
```{r qp}
